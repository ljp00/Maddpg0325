...
class UAVEnv:
    def __init__(self,length=2,num_obstacle=3,num_agents=4,grid_size = 100):
    ...
    def cal_rewards_dones(self,IsCollied,last_d):
        ...
        d_capture = 0.3
        d_limit = 0.75
        ## 1 reward for single rounding-up-UAVs:单次围捕无人机奖励
        for i in range(3):
            pos = self.multi_current_pos[i]
            vel = self.multi_current_vel[i]
        ...
        ## 2 collision reward for all UAVs:collision reward for all UAVs
        for i in range(self.num_agents):
            if IsCollied[i]:
                r_safe = -10
            else: 
    ...
    def render(self):
        ...
        for obstacle in self.obstacles:
            circle = plt.Circle(obstacle.position, obstacle.radius, color='black', alpha=0.5)
...
class obstacle():
    def __init__(self, length=2, is_dynamic=False):
        ...
        if is_dynamic:
            speed = np.random.uniform(0.01, 0.03)  # Random speed for dynamic obstacles
            self.velocity = np.array([speed * np.cos(angle), speed * np.sin(angle)])
        else:
            self.velocity = np.array([0.0, 0.0])  # Static obstacle with zero velocity
        self.radius = np.random.uniform(0.1, 0.15)
        self.is_dynamic = is_dynamic  # Store dynamic/static status
