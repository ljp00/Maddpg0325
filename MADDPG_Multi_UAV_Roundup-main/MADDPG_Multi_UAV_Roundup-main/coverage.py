...
class VisualizationSystem:
    def __init__(self, grid_size=(100, 100), num_uavs=4):
        self.grid_size = grid_size
        ...
        # 初始化左上：无人机轨迹图
        self.ax_traj.set_xlim(0, 2.5)#self.grid_size[0]
        self.ax_traj.set_ylim(0, 2.5)#self.grid_size[1]
        self.ax_traj.set_title('UAV Trajectories')

        # 初始化右上：热力图（背景固定，使用环境敏感度图）
        ...
        self.ax_heat.set_xlim(0, self.grid_size[0])
        self.ax_heat.set_ylim(0, self.grid_size[1])
        self.ax_heat.set_title('Environmental Sensitivity Heatmap')
        ...
        # 初始化左上轨迹跟踪（使用LineCollection实现连续轨迹，颜色映射由环境敏感度提供）
    ...
    def update_trajectories(self, positions):
        """
        positions: ndarray, shape (num_uavs, 2)
        """
        for i in range(self.num_uavs):
            ...
            if len(self.trajectories[i]) >= 2:
                ...
                # 通过环境敏感度映射给出每个轨迹点的颜色（这里可调整为其他映射策略）
                sensitivity_values = self.env_gen.get_sensitivity(self.trajectories[i])
...
if __name__ == '__main__':
    ...
    # 初始化自定义可视化系统（左上：轨迹；右上：热力图；下方：速度）
    vis_system = VisualizationSystem(grid_size=(100, 100), num_uavs=env.num_agents)
    ...
    def update(frame):
        ...
        # 对于速度，下方显示速度模长（可根据需要调整，下面直接用模长）
        velocities = [np.linalg.norm(env.multi_current_vel[i]) for i in range(env.num_agents)]

        # 更新自定义可视化系统：更新左上轨迹与下方速度曲线（右上热力图为静态背景）
